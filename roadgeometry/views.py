...
def get_nearby_road_segment(coord, num_points=10, radius=100):
    ...
    bbox = (lat - Decimal('0.01'), lon - Decimal('0.01'), lat + Decimal('0.01'), lon + Decimal('0.01'))
    query = f"""
    [out:json];
    ...
    road_segment_coords.sort(key=lambda c: ((c[0] - float(lat))**2 + (c[1] - float(lon))**2)**0.5) 
...
def calculate_radius(points):
    ...
    s = (d12 + d23 + d13) / 2
    radius = (d12 * d13 * d23) / (4 * (s * (s - d12) * (s - d13) * (s - d23))) ** 0.5
    return radius
